...
class Base
{
    ...
    protected function where($where, &$result)
    {
        ...
        foreach ($where as $item) {
            ...
            } else if ($item['type'] == 'enum' && !empty($item['name']) && !empty($item['value'])) {
                $this->applyEnumFilter($item['name'],$item['value'], $result);
            }
        }


        $ignoreTypeList = array_merge(['bool', 'primary', 'enum'], $this->additionalFilterTypeList);
    ...
    public function applyEnumFilter($filterName,$filterValue, &$result)
    {
        $this->prepareResult($result);
        
        $method = 'filter' . ucfirst($filterName); 
        if (method_exists($this, $method)) {
            $this->$method($filterValue,$result);
        }
    }
