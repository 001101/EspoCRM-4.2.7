...
class Application
{
    ...
    protected function routeConditions($method,$route,$crudList) {   
        $method = strtolower($method);

        if (!in_array($method, $crudList) && !in_array($method,['any'])) {
            $GLOBALS['log']->error('Route: Method ['.$method.'] does not exist. Please check your route ['.$route['route'].']');
            return;
        }
        
        if (in_array($method,['any'])) {
            $currentRoute = $this->getSlim()->any($route['route'], function() use ($route) {   //todo change "use" for php 5.4
                return $route['params'];
            });
        }
        else {
    ...
    protected function initRoutes()
    {
        ...
        foreach ($this->getRouteList() as $route) {
            ...
            foreach ($method as $val) {
                $this->routeConditions($val,$route,$crudList);
            }
