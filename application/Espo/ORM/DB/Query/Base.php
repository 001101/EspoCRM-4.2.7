...
abstract class Base
{
    ...
    public function createSelectQuery($entityName, array $params = array(), $deleted = false)
    {
        ...
        if (isset($params['removeWhereFields'])) {
            foreach ($params['removeWhereFields'] as $removeField) {
                if (array_key_exists($removeField, $whereClause)) {
                    unset($whereClause[$removeField]);
                }
            }
        }
    ...
    public function getKeys(IEntity $entity, $relationName)
    {
        ...
        switch ($relType) {
            ...
            case IEntity::HAS_CHILDREN:
                ...
                if (isset($relOpt['overrideWhere'])){
                    $overrideWhere = $relOpt['overrideWhere'];
                }
                $foreignKey = 'parentId';
                if (isset($relOpt['foreignKey'])) {
                    $foreignKey = $relOpt['foreignKey'];
