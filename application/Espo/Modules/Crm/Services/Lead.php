...
class Lead extends \Espo\Services\Record
{
    ...
    public function convert($id, $recordsData)
    {
        ...
        if ($memos = $lead->get('memos')) {
            foreach ($memos as $memo) {
                if (!empty($opportunity)) {
                    $memo->set('parentId', $opportunity->id);
                    $memo->set('parentType', 'Opportunity');
                    $entityManager->saveEntity($memo);
                } else if (!empty($contact)) {
                    $memo->set('parentId', $contact->id);
                    $memo->set('parentType', 'Contact');
                    $entityManager->saveEntity($memo);
                } else if (!empty($account)) {
                    $memo->set('accountId', $account->id);
                    $entityManager->saveEntity($memo);
                }
            }
        }
