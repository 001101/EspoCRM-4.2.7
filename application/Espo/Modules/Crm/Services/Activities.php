...
class Activities extends \Espo\Core\Services\Base
{
    ...
    protected function getMemoQuery($entity, $op = 'IN', $statusList = null)
    {
        $scope = $entity->getEntityType();
        $id = $entity->id;

        $methodName = 'get' .$scope . 'MemoQuery';
        if (method_exists($this, $methodName)) {
            return $this->$methodName($entity, $op, $statusList);
        }

        $selectManager = $this->getSelectManagerFactory()->create('Memo');
        ...
        $sql = $this->getEntityManager()->getQuery()->createSelectQuery('Memo', $selectParams);
        ...
        /*if ($this->isPerson($scope)) {
            ...
            if ($link) {
                ...
                $sql .= ' UNION ' . $this->getEntityManager()->getQuery()->createSelectQuery('Memo', $selectParams);
            }
        }*/
    ...
    protected function getTaskQuery($entity, $op = 'IN', $statusList = null)
    {
        $scope = $entity->getEntityType();
        $id = $entity->id;

        $methodName = 'get' .$scope . 'TaskQuery';
        if (method_exists($this, $methodName)) {
            return $this->$methodName($entity, $op, $statusList);
        }
        ...
        $sql = $this->getEntityManager()->getQuery()->createSelectQuery('Task', $selectParams);
    ...
    public function getActivities($scope, $id, $params = [])
    {
        ...
        if ($this->getAcl()->checkScope('Task')) {
            $parts['Task'] = ($fetchAll || $params['scope'] == 'Task') ? $this->getTaskQuery($entity, 'NOT IN', ['Canceled','Completed']) : [];
        }
    ...
    public function getHistory($scope, $id, $params)
    {
        ...
        if ($this->getAcl()->checkScope('Memo')) {
            $parts['Memo'] = ($fetchAll || $params['scope'] == 'Memo') ? $this->getMemoQuery($entity, 'IN', []) : [];
        }
        if ($this->getAcl()->checkScope('Task')) {
            $parts['Task'] = ($fetchAll || $params['scope'] == 'Task') ? $this->getTaskQuery($entity, 'IN', ['Canceled','Completed']) : [];
        }
