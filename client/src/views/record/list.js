...
Espo.define('views/record/list', 'view', function (Dep) {
    ...
    return Dep.extend({
        ...
        /**
         * @param {Bool} If true follow star will be shown.
         */
        follow: true,
        ...
        events: {
            ...
            'change .pagination select': function (e) {
                var maxSize = $(e.currentTarget).val();
                ...
                this.collection.fetch({maxSize:maxSize});
                this.collection.maxSize = parseInt(maxSize);
                this.trigger('paginate');
                this.deactivate();
            },
        ...
        init: function () {
            ...
            if (Espo.utils.isMobile.any() || (this.type != 'list' && this.type != 'listSmall') || (this.type == 'listSmall' && this.rowActionsView != false) ) {  
                this.pagination = false;
                this.basedOnRelation = true;
            }
        ...
        _loadListLayout: function (callback) {
            ...
            this._helper.layoutManager.get(this.collection.name, this.type, function (listLayout) {
                if (this.type === 'list') {
                    console.log("LIST LAYOUT ",listLayout);
                    this._helper.layoutManager.get(this.collection.name, 'listDefinitions', function (listDefinitions) {
                        this.listDefinitions = listDefinitions;
                        
                        if (this.listDefinitions.scrollTable == true)
                            this.listContainerEl = '.list > .table-responsive > table > tbody';
        ...
        _getHeaderDefs: function () {
            ...
            if (this.isIconList && this.rowActionsView) {
                defs.push({
                    width: this.iconRowActionsColumnWidth,
                });
            }            
        ...
        _convertLayout: function (listLayout, model) {
            ...
            if (this.isIconList && this.rowActionsView) {
                layout.push(this.getIconRowActionsDefs());
            }
        ...
        getIconRowActionsDefs: function (model) {
            return {
                name: 'icons',
        ...
        //actionFollow: function (id, $target) {
        actionListfollow: function (data) {
            console.log("ACTION FOLLOW LIST!!");
            data = data || {}
            var id = data.id;
            if (!id) return;

            $target = this.$el.find('.action.followstar[data-id="' + id + '"]');
            var $el = $target.find('.star');
            $target.addClass('disabled');
            
            var model = this.collection.get(id);
            
            if ($target.data('follow') == true) {
                console.log('UNFFOOLLOW');
                // UNFOLLOW
                $.ajax({
                    url: model.name + '/' + id + '/subscription',
                    type: 'DELETE',
                    success: function () {
                        $el.removeClass('filled');
                        $target.data('follow',false);
                        model.set('isFollowed', false);
                        console.log('UNFFOOLLOW success');
                        $target.removeClass('disabled');
                        $target.focusout();
                    }.bind(this),
                    error: function () {
                        console.log('UNFFOOLLOW error');
                        $target.removeClass('disabled');
                    }.bind(this)
                });
            }
