...
Espo.define('views/record/detail', ['views/record/base', 'view-record-helper'], function (Dep, ViewRecordHelper) {
    ...
    return Dep.extend({
        ...
        menuView: 'views/record/detail-menu',
        ...
        showPanel: function (name) {
            ...
            var menuView = this.getView('menu');
            if (menuView) {
                 menuView.showPanel(name);
            }
        ...
        hidePanel: function (name) {
            ...
            var menuView = this.getView('menu');
            if (menuView) {
                menuView.hidePanel(name);
            }
            
            var middleView = this.getView('middle');
            if (middleView) {
                console.log("HIDE PANEL MIDDLE");
        ...
        setupFinal: function () {
            ...
            console.log("MODEL IS",this.model.name,this.mode,this.sidebarEnabledList.indexOf(this.model.name));
            if (this.sidebarEnabledList.indexOf(this.model.name) != -1) {
                this.hasSide = true;
            }
            
            this.hasSide = this.getMetadata().get('clientDefs.' + this.scope + '.hasSide') || this.hasSide;
        ...
        convertDetailLayout: function (simplifiedLayout) {
            ...
            for (var p in simplifiedLayout) {
                ...
                panel.alwaysVisible = simplifiedLayout[p].alwaysVisible || false;
                panel.name = simplifiedLayout[p].name || Espo.Utils.composeCssName(simplifiedLayout[p].label);
                panel.rows = [];
                console.log(simplifiedLayout[p].rows);
                console.log("CREATE PANEL",panel.label);
        ...
        getGridLayout: function (callback) {
            ...
            if (this.detailLayout) {
                console.log("DETAIL LAYOUT",this.detailLayout);
                this.gridLayout = {
                    type: 'record',
            ...
            this._helper.layoutManager.get(this.model.name, this.layoutName, function (simpleLayout) {
                console.log("SIMPLELAYOUT",simpleLayout);
                this.gridLayout = {
                    type: 'record',
        ...
        createSideView: function () {
            ...
            this.createView('side', this.sideView, {
                ...
                type: this.type,
                mode: this.mode,
                activeMenuPanel: this.activePanel,
        ...
        createMiddleView: function (callback) {
            console.log("BUILD MIDDLE");
            var el = this.options.el || '#' + (this.id);
            this.waitForView('middle');
            this.getGridLayout(function (layout) {
                var panelList = layout.layout.map(function (p) {
                    var item = Espo.Utils.clone(p);
                    var item = {
                        label: p.label,
                        model: this.model,
                        mode: this.mode,
                        name: Espo.Utils.composeCssName(p.label),
                        //shortName: 'tabpanel-' + Espo.Utils.composeCssName(p.label),
                        shortName: 'tabpanel-' + Espo.Utils.composeCssName(this.scope) + '-' + Espo.Utils.composeCssName(p.label),
                        recordHelper: this.recordHelper,
                        defs: item
                    };
                    return item;
                ...
                this.createView('menu', this.menuView, {
                    model: this.model,
                    mode: this.mode,
                    type: this.type,
                    panelList: panelList,
        ...
        loadStored: function () {
            console.log("MENU LOAD STORED",this.type + 'Menu' + this.scope);
            ...
            additionalPanels.forEach(function (panel) {
                if (panel.name == this.defaultPanel)
                    foundDefault = true;
            }, this);
            ...
            this.activePanel = this.getStorage().get(this.type + 'Menu' + this.scope, 'activePanel') || Espo.Utils.clone(this.defaultPanel);
        }
